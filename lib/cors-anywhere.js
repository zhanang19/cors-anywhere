...
var token = process.env.ZHA_TOKEN;
...
function parseURL(req_url) {
  ...
  var req_url_length = req_url.lastIndexOf('/');
  var real_url = req_url.substring(0, req_url_length);
  var last = req_url.substring(req_url_length, req_url.length);
  if(last === "/" + token ) {
    // return the url
